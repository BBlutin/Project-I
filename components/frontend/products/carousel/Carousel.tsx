...
const Carousel = ({images, options}: Props) => {
    ...
    return (
        <>
            <div className="flex h-[70vh] flex-col product-sticky">
                <div className="overflow-hidden h-5/6" ref={emblaMainRef}>
                    <div className="product__container">
                    {images.map((image, index) => (
                        <div className="product__slide" key={index}>
                            <Image src={urlFor(image).url()} className="object-contain object-center rounded-md" fill alt="Image du produit" />
                        </div>
                    ))}
                    </div>
                </div>

                <div className="px-4 mt-8 h-1/6">
