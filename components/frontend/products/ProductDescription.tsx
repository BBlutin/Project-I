            ...
            <div className="block text-neutral-600 dark:text-neutral-400 mb-1.5 md:mb-2.5">
                { product.discount ? (
                    <span className='block mt-1 mb-4 text-grey-900 dark:text-white'>
                        <span className='font-semibold text-green-600 dark:text-green-400'>{product.discount} €</span>
                        <span className='ml-1 font-medium line-through text-neutral-500'>{product.price} €</span>
                    </span>
                ) : (
                ...
                { sizes && (
                    <>
                        ...
                        <div className="grid w-full grid-cols-5 gap-1">
                            {product.sizes.map((size) => {
                                let inStock = false
                                if (size.quantity > 0) { inStock = true }
                                return (
                                    <div
                                        key={size._key}
                                        onClick={() => handleSizeChange(size._key)}
                                        className={`relative z-10 flex items-center justify-center h-10 text-xs font-medium border rounded cursor-pointer border-neutral-200 dark:border-neutral-700 ${size._key === activeSize ? 'border-neutral-800 dark:border-neutral-100' : 'hover:border-neutral-400 hover:dark:border-neutral-500'}`}
                                    >
                                        <span className={!inStock ? "text-neutral-300 dark:text-neutral-500" : ""}>{size.name}</span>
                                        {!inStock &&
